...
let img = [];
const url = 'YOUR SCRAPPING URL';
var grps = wa_grp_id.split(',');
...
async function startMiku() {
    ...
    console.log(color(figlet.textSync('WA NEWS MD', {
        font: 'Pagga',
    ...
    console.log(color('\nHello, I am Fazil, the main developer of this bot.\n\nThanks for using: WA NEWS MD made by my Fazil vk.', 'aqua'))
    console.log(color('\nYou can follow me on GitHub: mu-fazil-vk\n\n', 'aqua'))
    ...
    const Miku = MikuConnect({
        ...
        browser: ['WA-NEWS-MD', 'Safari', '1.0.0'],
        auth: state,
        version
    })
    ...
    Miku.ev.on('connection.update', async (update) => {
        ...
        status = connection;
        if (connection) {
            console.info(`WA NEWS MD Server Status => ${connection}`);
        }

        if (connection === 'open') {
            await Miku.sendMessage(`${owner}@s.whatsapp.net`, { text: `Periodic news check initiated ✨` });
            periodicCheck();
        }
    ...
    //news automate
    async function checkManorama() {

        console.log("Started news check for manorama");

        const response1 = await axios.get(url + '/manorama');
        const data1 = response1.data;
        const image1 = data1.image;
        const locnews1 = data1.news;
        let news1 = 0;

        console.log(img);
        if (img.length === 0) {
            console.log("no initial manorama news");
            img.push(image1);
        } else {
            for (let i = 0; i < img.length; i++) {
                if (image1 != img[i] && image1 != '') {
                    news1 = 1;
                } else {
                    news1 = 0;
                    break;
        ...
        if (news1 === 1) {
            for (var i = 0; i < grps.length; i++) {
                await Miku.sendMessage(grps[i], { image: { url: image1 }, caption: locnews1 });
            }
            img.push(image1);
        } else {
            console.log("No new news from Manorama");
        }

    }

    async function checkMangalam() {

        console.log("Started news check for mangalam");

        const response2 = await axios.get(url + '/mangalam');
        const data2 = response2.data;
        const image2 = data2.image;
        const locnews2 = data2.news;
        let news2 = 0;

        if (img.length === 1) {
            console.log("no initial mangalam news");
            img.push(image2);
        } else {
            for (let i = 0; i < img.length; i++) {
                if (image2 != img[i] && image2 != '') {
                    news2 = 1;
                } else {
                    news2 = 0;
                    break;
        ...
        if (news2 === 1) {
            for (var i = 0; i < grps.length; i++) {
                await Miku.sendMessage(grps[i], { image: { url: image2 }, caption: locnews2 });
            }
            img.push(image2);
        } else {
            console.log("no new mangalam news");
        }
    }

    function periodicCheck() {
        try {
            checkManorama().then(() => checkMangalam());
        } catch (error) {
            console.error('Error in periodic news check:', error);
        }

        // Schedule the next check after a certain interval (e.g., 1 minute)
        setTimeout(periodicCheck, 60000);
    }
